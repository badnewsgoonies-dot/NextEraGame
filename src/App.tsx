...
export function App(): React.ReactElement {
  ...
  const handleRecruit = (enemyId: string, replaceUnitId?: string) => {
    ...
    if (recruitResult.ok) {
      ...
      const transitionResult = controller.getStateMachine().transitionTo('roster_management');
      if (!transitionResult.ok) {
        console.error('Failed to transition to roster_management:', transitionResult.error);
        return;
      }
  ...
  const handleSkipRecruit = () => {
    // Create roster from current team (even if no recruitment)
  ...
  const handleRosterContinue = () => {
    ...
    const validationResult = rosterManager.validateRoster(roster);
    if (!validationResult.ok) {
      console.error('Invalid roster:', validationResult.error);
      return;
    }
    
    // Sync playerTeam with active party (ensure consistency)
